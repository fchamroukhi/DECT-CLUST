...
function [indx,ssw,sw,sb]=valid_clusterIndex(data,labels,mean_curves)
% clustering validation indices 
% Kaijun WANG, sunice9@yahoo.com, May 2005, Oct. 2006
[nr,nc]=size(data);
k=max(labels);
if ~isempty(mean_curves)    % spectral
    [st,sw,sb,S,Sinter] = valid_sumsqures_curves(data,labels,k,mean_curves);
else                        % spatial
